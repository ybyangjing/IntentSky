        ...
        if r['waiting_policy'] == 'zero-1':
            ...
            if r['loop'] == 1 and r['max_queue_length'] == 30:
                if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
                    temp.append(r)
        elif r['waiting_policy'] == 'cardinal_query-1':
             if r['loop'] == 1 and r['max_queue_length'] == 30:
                if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
                    temp.append(r)
        elif r['waiting_policy'] == 'heft-1':
            if r['loop'] == 0 and r['preempt_cloud_ratio'] == -1 and r['long_job_thres'] == -1:
                temp.append(r)
    return temp
...
def philly_end2end_filter(run_configs):
    ...
    for r in run_configs:
        if r['waiting_policy'] == 'zero-1':
            ...
            if r['loop'] == 0 and r['max_queue_length'] == 1000000 and r['long_job_thres']==-1 and r['preempt_cloud_ratio'] == -1:
                temp.append(r)
        elif r['waiting_policy'] == 'infinite-1' and r['preempt_cloud_ratio'] == -1:
            # Constant Wait and Constant-Wait No-SJ
            if r['loop'] == 0 and r['max_queue_length'] == 1000000:
                temp.append(r)
        elif r['waiting_policy'] == 'linear_runtime-1' and r['preempt_cloud_ratio'] == -1:
            # Constant Wait and Constant-Wait No-SJ
            if r['loop'] == 1 and r['max_queue_length'] == 30:
                temp.append(r)
        elif r['waiting_policy'] == 'quad_capacity-1' and r['preempt_cloud_ratio'] == -1:
            # Constant Wait and Constant-Wait No-SJ
            if r['loop'] == 1 and r['max_queue_length'] == 30:
                temp.append(r)
        elif r['waiting_policy'] == 'constant-1' and r['preempt_cloud_ratio'] == -1:
            ...
            if r['loop'] == 1 and r['max_queue_length'] == 30:
                if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
                    temp.append(r)
        # elif r['waiting_policy'] == 'cardinal_query-1' and r['loop'] == 0 and r['long_job_thres'] > 0 and r[
        #     'preempt_cloud_ratio'] != -1:
        #     temp.append(r)
        elif r['waiting_policy'] == 'cardinal_query-0.076':
            # Starburst without Time Estimator (Star Wait + 00)
            if r['loop'] == 1 and r['max_queue_length'] == 30:
                if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
                    temp.append(r)
             ...
             if r['loop'] == 1 and r['max_queue_length'] == 30:
                if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
                    temp.append(r)
        # Baselines for Philly End2End
        elif r['waiting_policy'] == 'round_robin-1':
            if r['loop'] == 0 and r['preempt_cloud_ratio'] == -1 and r['long_job_thres'] == -1:
                temp.append(r)
        elif r['waiting_policy'] == 'k8s_best_fit-1':
            if r['loop'] == 0 and r['preempt_cloud_ratio'] == -1 and r['long_job_thres'] == -1:
                temp.append(r)
        elif r['waiting_policy'] == 'heft-1':
            if r['loop'] == 0 and r['preempt_cloud_ratio'] == -1 and r['long_job_thres'] == -1:
...
def philly_end2end_pareto(run_configs):
    ...
    for r in run_configs:
        ...
        if asdf == 'constant' and r['loop'] == 0 and r['preempt_cloud_ratio'] == -1:
            ...
            temp.append(r)
        # elif asdf == 'cardinal_query' and r['loop'] == 0 and r['long_job_thres'] > 0 and r[
        #     'preempt_cloud_ratio'] != -1:
        #     temp.append(r)
        elif asdf == 'cardinal_query' and r['loop'] == 1 and r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
            # Starburst without Time Estimator (Star Wait + OO)
            temp.append(r)
        elif asdf == 'heft' and r['loop'] == 0:
            temp.append(r)
    return temp
...
def helios_end2end_pareto(run_configs):
    ...
    for r in run_configs:
        ...
        if asdf == 'constant' and r['loop'] == 0 and r['preempt_cloud_ratio'] == -1:
            ...
            #  Starburst (Compute Wait + OO) - Added for consistency
            temp.append(r)
        elif asdf == 'linear_capacity_filter_cpu' and r['loop'] == 1 and r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
            # Starburst without Time Estimator (Star Wait + OO)
            temp.append(r)
        elif asdf == 'linear_capacity' and r['loop'] == 1 and r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
            # Starburst without Time Estimator (Star Wait + OO) - Added for consistency
            temp.append(r)
        # elif r['waiting_policy'] == 'cardinal_query' and r['loop'] == 0 and r['long_job_thres'] > 0 and r[
        #     'preempt_cloud_ratio'] != -1:
        #     temp.append(r)
        elif asdf == 'cardinal_query' and r['loop'] == 1 and r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
            # Starburst without Time Estimator (Star Wait + OO)
            ...
            # HEFT baseline
            if r['preempt_cloud_ratio'] == -1 and r['long_job_thres'] == -1:
                temp.append(r)
    return temp
...
def ablate_waiting_policy_fiter(run_configs):
    ...
    for r in run_configs:
        if r['waiting_policy'] == 'zero-1' and r['preempt_cloud_ratio'] == -1 and r['long_job_thres']==-1:
            ...
            # Starburst (Compute Wait + OO)
            temp.append(r)
        elif r['waiting_policy'] == 'cardinal_query-0.04' and r['preempt_cloud_ratio'] == -1 and r['long_job_thres']==-1:
            # Starburst (Compute Wait + OO)
            temp.append(r)
            ...
            if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] > 0:
                ...
                # Resource-Wait + OO
                temp.append(r)
        # elif r['waiting_policy']=='cardinal_query-1' and r['loop'] == 0 and r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] != -1:
        #     temp.append(r)
        elif r['waiting_policy'] == 'cardinal_query-0.234':
            # cardinal_query
            if r['long_job_thres'] > 0 and r['preempt_cloud_ratio'] > 0:
                temp.append(r)
            elif r['long_job_thres']==-1 and r['preempt_cloud_ratio'] == -1:
...
def ablate_out_of_order_filter(run_configs):
    ...
    for r in run_configs:
        ...
        #     'preempt_cloud_ratio'] != -1:
        #     temp.append(r)
        elif (asdf == 'cardinal_query' or asdf == 'cardinal_query') and r['preempt_cloud_ratio'] > 0 and r['long_job_thres'] > 0:
            # Starburst without Time Estimator (Star Wait)
            if 'max_queue_length' in r:
                if r['max_queue_length'] < 100:
                    temp.append(r)
...
def ablate_jct_percentile(run_configs):
    ...
    for r in run_configs:
        ...
        #     'preempt_cloud_ratio'] != -1:
        #     temp.append(r)
        elif asdf == 'cardinal_query' and r['loop'] == 1 and r['preempt_cloud_ratio'] > 0 and r['long_job_thres'] > 0 and r['max_queue_length'] == 10:
            temp.append(r)
    return temp
...
def philly_waiting_budget_pareto_filter(run_configs):
    ...
    for r in run_configs:
        ...
        if asdf == 'constant' and r['loop'] == 1 and r['preempt_cloud_ratio'] == -1 and r['long_job_thres']==-1:
            ...
            temp.append(r)
        elif asdf == 'cardinal_query' and r['loop'] == 1 and r['preempt_cloud_ratio'] > 0 and r['long_job_thres'] > 0:
            temp.append(r)
        # elif asdf == 'cardinal_query' and r['loop'] == 0 and r['long_job_thres'] > 0 and r[
        #     'preempt_cloud_ratio'] != -1:
        #     temp.append(r)
...
def ablate_job_offload_filter(run_configs):
    ...
    for r in run_configs:
        ...
        #     if r['max_queue_length'] == 1000000:
        #         temp.append(r)
        elif r['waiting_policy'] == 'cardinal_query-0.04':
            temp.append(r)
    return temp
